...
if st.button("Maak teams"):
    ...
    # Bepaal eerlijke teamgroottes (zo gelijk mogelijk)
    ...
    for _ in range(10000):
        ...
        # Verdeel veldspelers volgens team_sizes
        pointers = [len(team) for team in temp_teams]  # beginstand per team (na keepers)
        team_indices = []
        for i, size in enumerate(team_sizes):
            team_indices.extend([i] * (size - pointers[i]))  # vul tot gewenste grootte

        random.shuffle(team_indices)  # random verdeling over juiste aantallen
